module.exports = {  // database의 data query로 가져오기
    ...
    productDetail2: {    // t_product에서 해당 id를 가진 제품의 필요한 data만 가져오는 쿼리
    ...
    productMainImages: {    // type이 2인 해당 id를 가진 이미지를 가져오는 쿼리
        query: `SELECT * FROM t_image WHERE product_id = ? AND type = 2`
    },
    productInsert: {    // 받아온 파라미터들을 t_product에 Insert하는 쿼리
        query: `INSERT INTO t_product SET ?`
    },
    productImageInsert: {   // 받아온 파라미터들을 t_image에 Insert하는 쿼리
        query: `INSERT INTO t_image SET ?`
    },
    productUpdate: {    // 해당 id를 가진 제품의 data를 받아온 파라미터들로 Update하는 쿼리
        query: `UPDATE t_product SET ? WHERE id = ?`
    },
    imageList: {    // 해당 id를 가진 제품의 이미지 가져오는 쿼리
        query: `SELECT * FROM t_image WHERE product_id=?`
    },
    imageDelete: {  // 해당 id를 가진 이미지 삭제하는 쿼리
        query: `DELETE FROM t_image WHERE id=?`
    },
    productDelete: {    // 해당 id를 가진 제품 삭제하는 쿼리
        query: `DELETE FROM t_product WHERE id=?`  
    },
    categoryList: {   // 카테고리 리스트 갸져오는 쿼리
        query: `SELECT * FROM t_category`     // t_category 테이블의 데이터 가져오기
    },
    sellerList: {   // 셀러 리스트 가져오는 쿼리
        query: `SELECT * FROM t_seller`     // t_seller 테이블의 데이터 가져오기
    },
    memberJoin: {    // 유저 회원가입을 위한 동일한 email이 있는지 찾는 쿼리
    ...
    cartDelete: {    // 해당 id를 가진 장바구니 정보 장바구니에서 삭제하는 쿼리
        query: `DELETE FROM t_cart WHERE cart_id=?`
    },
    userInfo: {    // 해당 email를 가진 회원 정보를 가져오는 쿼리
        query: `SELECT * FROM t_user WHERE email = ?`
    },
    sellerInfo: {    // t_seller 정보를 가져오는 쿼리
        query: `SELECT * FROM t_seller`
    },
    orderPayment: {    // 받아온 파리미터들을 t_order에 Insert하는 쿼리
        query: `INSERT INTO t_order (address, name, phone, requested_term, name_of_depositor, user_email, payment_amount) VALUES (?, ?, ?, ?, ?, ?, ?)`
    },
    cartCopy: {    // 해당 email를 가진 t_cart data를 t_order로 복사하는 쿼리
    ...
    deleteCart: {    // 해당 email를 가진 t_cart data 삭제하는 쿼리
        query: `DELETE FROM t_cart WHERE user_email = ?`
    },
    orderList: {    // 해당 email를 가진 주문내역 정보 가져오기
    ...
    orderCancle: {  // 해당 cart_id를 가진 data의 delivery_state를 3으로 변경하여 주문취소로 바꿔주는 쿼리
        query: `UPDATE t_orders SET delivery_status = 3 WHERE cart_id = ?`
    },
    userManagement: {   // t_user의 data를 가져오는 쿼리
        query: `SELECT * FROM t_user`
    },
    orderManagement: {  // 모든 주문 정보를 가져오는 쿼리
    ...
    orderDetail: {  // 해당 order_id를 가진 주문내역 상세 정보 가져오기
